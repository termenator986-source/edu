...
def make_main_menu_keyboard() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🎲 Испытать удачу — скидка 20%!", callback_data="menu_dice")],
...
async def menu_dice_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if value == 3:
        ...
        text = (
            f"🎲 Выпало: {value}!\n\n"
            "🎉 УДАЧА! Вы выбросили 3!\n\n"
            "🔥 Вам начислена скидка 20% на весь заказ!\n"
            "Скидка применится автоматически при оплате.\n\n"
            "👇 Нажмите «Купить ответы» чтобы воспользоваться скидкой!"
        )
    else:
        context.user_data["dice_discount"] = False
        text = (
            f"🎲 Выпало: {value}\n\n"
            "😔 Не повезло, нужна тройка...\n\n"
            "Но не расстраивайся! У нас есть акция:\n"
            "🔥 Мат + Рус + 2 предмета = 1790 ₽\n\n"
            "⏰ Следующий бросок доступен через 24 часа."
        )
    markup = InlineKeyboardMarkup([
        [InlineKeyboardButton("🛒 Купить ответы", callback_data="menu_buy")],
