...
def make_main_menu_keyboard() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🎲 Испытать удачу!", callback_data="menu_dice_info")],
...
async def show_main_menu(query, context, user):
    ...
    text = (
        ...
        "━━━━━━━━━━━━━━━━━━━━\n"
        "🎲 ИГРА: Брось кость — выпадет 3️⃣ = скидка 20%!"
        f"{free_line}"
    )
...
async def menu_dice_info_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        "🎲 ИСПЫТАЙ УДАЧУ\n"
        "━━━━━━━━━━━━━━━━━━━━\n\n"
        "Брось кубик и выиграй скидку 20% на любой заказ!\n\n"
        "🎯 Правила:\n"
        "   Выбрось 3️⃣  →  скидка 20% твоя!\n"
        "   Другое число  →  попробуй завтра\n\n"
        "⚡️ Одна попытка в 24 часа\n"
        "━━━━━━━━━━━━━━━━━━━━\n\n"
        "Готов? Жми кнопку и бросай! 🤞"
    )
    markup = InlineKeyboardMarkup([
        [InlineKeyboardButton("🎲 Бросить кубик!", callback_data="menu_dice")],
        [InlineKeyboardButton("◀️ Назад", callback_data="menu_main")],
    ])
    await query.edit_message_text(text, reply_markup=markup)
    return SELECT_REGION
...
async def menu_dice_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await notify_admin(context.bot, f"🎲 {user_tag(user)} бросает кость")
    ...
    if value == 3:
        ...
        text = (
            f"🎲 РЕЗУЛЬТАТ\n"
            "━━━━━━━━━━━━━━━━━━━━\n\n"
            f"Выпало: {value} — не повезло 😔\n\n"
            "Нужна тройка, но не сдавайся!\n\n"
            "Пока действует акция:\n"
            "🔥 Мат + Рус + 2 предмета = 1790 ₽\n\n"
            "⏰ Новый бросок — завтра!"
        )
    markup = InlineKeyboardMarkup([
        [InlineKeyboardButton("🛒 Купить ответы", callback_data="menu_buy")],
...
async def pay_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    dice_note = "\n🎲 Применена скидка 20% за удачный бросок кости!" if dice_discount else ""
    text = (
        "╔══════════════════════╗\n"
        "        💳 ОПЛАТА ЗАКАЗА\n"
        "╚══════════════════════╝\n\n"
