...
ADMIN_USER_ID = int(os.environ.get("ADMIN_USER_ID", "0"))  # личный ID для /broadcast
ANTHROPIC_API_KEY = os.environ["ANTHROPIC_API_KEY"]
CARD_NUMBER = "2204 3204 6631 3501"
PRICE_PER_SUBJECT = 549
PROMO_PRICE = 1590
PROMO_SUBJECTS_COUNT = 4
CONTACT_LINK = "https://t.me/Manager_ogge2026"
...
def calc_cart(cart: list, dice_discount: bool = False) -> tuple:
    ...
    if has_math and has_rus and len(cart) >= PROMO_SUBJECTS_COUNT:
        ...
        base = PROMO_PRICE + extra * PRICE_PER_SUBJECT
    else:
        base = len(cart) * PRICE_PER_SUBJECT
    promo = has_math and has_rus and len(cart) >= PROMO_SUBJECTS_COUNT
    if dice_discount and base > 0:
        base = round(base * 0.8)
    return base, promo
...
def cart_text(cart: list) -> str:
    ...
    promo_line = "\n🔥 Применена акция: Рус + Мат + 2 предмета = 1 590 ₽" if promo else ""
...
def make_main_menu_keyboard() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🏀 Бросить в кольцо!", callback_data="menu_dice_info")],
        [InlineKeyboardButton("👤 Личный кабинет", callback_data="menu_cabinet")],
        [InlineKeyboardButton("🆘 Техподдержка", url="https://t.me/Manager_ogge2026")],
    ])
...
async def show_main_menu(query, context, user):
    ...
    text = (
        "🏠 Главное меню\n\n"
        "📋 Реальные ответы на ОГЭ 2026\n"
        "по всем регионам России\n\n"
        "✅ Более 3 000 довольных учеников\n"
        "✅ Ответы строго по вашему региону\n"
        "✅ К каждому предмету прилагаются КИМы\n"
        "   — сравни ответы и лучше подготовься!\n\n"
        "📲 После оплаты вас добавят в беседу\n"
        "   вашего региона и выбранного предмета\n\n"
        "⏰ Ответы приходят за 1 день до экзамена\n"
        "   в 21:00 по московскому времени\n\n"
        "🔥 АКЦИЯ: Мат + Рус + 2 предмета = 1 590 ₽\n"
        f"💰 Или 1 предмет всего за {PRICE_PER_SUBJECT} ₽\n\n"
        "🏀 ИГРА: Брось в кольцо — попадёшь = скидка 20%!"
        f"{free_line}"
    )
...
async def show_cabinet(query, context, user):
    ...
    if query.message.photo:
        try:
            await query.edit_message_caption(caption=text, reply_markup=make_cabinet_keyboard(has_free=free))
        except Exception:
            await query.message.delete()
            if os.path.exists(photo_path):
                with open(photo_path, "rb") as photo:
                    await query.message.chat.send_photo(photo=photo, caption=text, reply_markup=make_cabinet_keyboard(has_free=free))
            else:
                await query.message.chat.send_message(text=text, reply_markup=make_cabinet_keyboard(has_free=free))
    else:
        await query.message.delete()
        if os.path.exists(photo_path):
            with open(photo_path, "rb") as photo:
...
async def captcha_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chosen == correct:
        ...
        caption = (
            ...
            "⏰ Ответы приходят за 1 день до экзамена\n"
            "   в 21:00 по московскому времени\n\n"
            "🔥 АКЦИЯ: Математика + Русский + 2 предмета = 1 590 ₽\n"
            f"💰 Или 1 предмет всего за {PRICE_PER_SUBJECT} ₽"
            f"{free_line}\n\n"
            "👇 Выберите действие:"
        )
...
async def region_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        f"📍 Регион: {region}\n\n"
        f"🔥 АКЦИЯ: Математика + Русский + 2 предмета = 1 590 ₽\n"
...
async def menu_dice_info_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Экран с описанием игры — перед броском."""
    query = update.callback_query
    await query.answer()
    user = update.effective_user
    ...
    uid = str(user.id)
    now = time.time()
    last_dice = users.get(uid, {}).get("last_dice", 0)
    seconds_left = int(last_dice + 86400 - now)
    ...
    if seconds_left > 0:
        ...
        text = (
            "🏀 ИСПЫТАЙ УДАЧУ\n\n"
            "⏳ Ты уже бросал сегодня.\n\n"
            f"Следующий бросок через:\n"
            f"🕐 {hours} ч. {minutes} мин.\n\n"
            "Возвращайся завтра — удача ждёт тебя!"
        )
        markup = InlineKeyboardMarkup([
            [InlineKeyboardButton("🛒 Купить ответы", callback_data="menu_buy")],
    ...
    text = (
        "🏀 ИСПЫТАЙ УДАЧУ\n\n"
        "Брось мяч и выиграй скидку 20% на любой заказ!\n\n"
        "🎯 Правила:\n"
        "   Попади в кольцо  →  скидка 20% твоя!\n"
        "   Промах  →  попробуй завтра\n\n"
        "⚡️ Одна попытка в 24 часа\n\n"
        "Готов? Жми кнопку и бросай! 🤞"
    )
    markup = InlineKeyboardMarkup([
        [InlineKeyboardButton("🏀 Бросить в кольцо!", callback_data="menu_dice")],
        [InlineKeyboardButton("◀️ Назад", callback_data="menu_main")],
    ])
    await send_or_edit(query, text, markup)
    return SELECT_REGION
...
async def menu_dice_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Мини-игра: бросок кости — раз в 24 часа."""
    import asyncio
    query = update.callback_query
    await query.answer()
    user = update.effective_user
    ...
    if seconds_left > 0:
        ...
        minutes = (seconds_left % 3600) // 60
        await query.answer(
            f"⏳ Бросок доступен через {hours}ч {minutes}мин",
            show_alert=True
        )
        return SELECT_REGION

    if uid in users:
        users[uid]["last_dice"] = now
        save_users(users)

    await notify_admin(context.bot, f"🏀 {user_tag(user)} бросает в кольцо")
    dice_msg = await context.bot.send_dice(chat_id=query.message.chat_id, emoji="🏀")
    value = dice_msg.dice.value
    await asyncio.sleep(3)

    if value == 5:
        context.user_data["dice_discount"] = True
        text = (
            "🏀 РЕЗУЛЬТАТ\n\n"
            "🎉 ПОПАДАНИЕ В КОЛЬЦО — ТЫ ПОБЕДИЛ!\n\n"
            "🔥 Скидка 20% начислена!\n"
            "Она применится автоматически при оплате.\n\n"
            "👇 Нажми «Купить ответы» и оформляй заказ!"
        )
    else:
        context.user_data["dice_discount"] = False
        text = (
            "🏀 РЕЗУЛЬТАТ\n\n"
            "Промах 😔 Мяч не попал в кольцо.\n\n"
            "Нужно попадание, но не сдавайся!\n\n"
            "Пока действует акция:\n"
            "🔥 Мат + Рус + 2 предмета = 1 590 ₽\n\n"
            "⏰ Новый бросок — завтра!"
        )
    markup = InlineKeyboardMarkup([
        [InlineKeyboardButton("🛒 Купить ответы", callback_data="menu_buy")],
...
async def pay_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cart = context.user_data.get('cart', [])
    subjects_names = [SUBJECTS[i] for i in cart]
    dice_discount = context.user_data.get("dice_discount", False)
    total, promo = calc_cart(cart, dice_discount)
    ...
    promo_note = "\n🔥 Применена акция: Мат + Рус + 2 предмета = 1 590 ₽" if promo else ""
    dice_note = "\n🏀 Применена скидка 20% за попадание в кольцо!" if dice_discount else ""
    text = (
        "╔══════════════════════╗\n"
        "        💳 ОПЛАТА ЗАКАЗА\n"
        "╚══════════════════════╝\n\n"
        ...
        "👤 Получатель:\n"
        "   Артемьев Даниил Алексеевич\n\n"
        f"🏦 Номер карты:\n"
        f"   {CARD_NUMBER}\n\n"
        f"💰 Сумма к оплате: {total} ₽"
        f"{promo_note}{dice_note}\n"
        "━━━━━━━━━━━━━━━━━━━━\n\n"
        "✅ Как оплатить:\n"
        "1️⃣ Откройте приложение банка\n"
        "2️⃣ Переведите точную сумму на карту\n"
        "3️⃣ Сделайте скриншот чека\n"
        "4️⃣ Отправьте скриншот сюда 👇\n\n"
        "⚡️ Проверка — автоматически за 10 секунд\n"
...
async def cabinet_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if os.path.exists(photo_path):
        with open(photo_path, "rb") as photo:
            await update.message.reply_photo(photo=photo, caption=text, reply_markup=make_cabinet_keyboard(has_free=free))
    else:
        await update.message.reply_text(text, reply_markup=make_cabinet_keyboard(has_free=free))


async def broadcast_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Рассылка всем пользователям. Только для админа (/broadcast текст)."""
    import asyncio
    user = update.effective_user
    ...
    if not text:
        await update.message.reply_text(
            "📢 Использование:\n/broadcast Текст сообщения\n\nПример:\n/broadcast 🔥 Успей купить ответы до экзамена!"
        )
        return
    users = load_users()
    total = len(users)
    sent = 0
    failed = 0
    status_msg = await update.message.reply_text(f"📤 Начинаю рассылку {total} пользователям...")
    for uid in users:
        try:
    ...
    conv = ConversationHandler(
        ...
        states={
            ...
            SELECT_REGION: [
                ...
                CallbackQueryHandler(menu_dice_info_cb, pattern="^menu_dice_info$"),
                CallbackQueryHandler(menu_dice_cb, pattern="^menu_dice$"),
